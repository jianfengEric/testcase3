...
public class PageQuery<T> implements Serializable{
    ...
    public void getResult(Class<T> clazz){
        StringBuilder hql = new StringBuilder("from ").append(clazz.getSimpleName()).append(" a where 1=1");
        parameters = new HashMap<>();
        if(!getMustQueries().isEmpty()){
            hql.append(" and ( ");
            hql.append(builderHQL(getMustQueries(), Criteria.AND));
            hql.append(" ) ");
        }
        if(!getFieldQueries().isEmpty()){
            hql.append(" and ( ");
            hql.append(builderHQL(getFieldQueries(),criteria));
            hql.append(" ) ");
        }
        if(!getSortQueries().isEmpty()){
            hql.append(" order by ");
            int i = 0;
